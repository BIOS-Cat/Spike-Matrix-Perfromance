...
static inline vec3 *cross_VERSION_A(vec3 *result, const vec3 *const u, const vec3 *const v)
{
...
static inline vec3 *scale3_VERSION_A(vec3 *result, float s, const vec3 *const v)
{
    result->v[0] = s * v->v[0];
    result->v[1] = s * v->v[1];
    result->v[2] = s * v->v[2];
...
static inline vec4 *scale4_VERSION_A(vec4 *result, float s, const vec4 *const v)
{
    result->v[0] = s * v->v[0];
    result->v[1] = s * v->v[1];
    result->v[2] = s * v->v[2];
...
static inline vec3 *normalize3_VERSION_A(vec3 *result, const vec3 *const v)
{
    return scale3_VERSION_A(result, 1.0f / norm3(v), v);
}

static inline vec4 *normalize4_VERSION_A(vec4 *result, const vec4 *const v)
{
    return scale4_VERSION_A(result, 1.0f / norm4(v), v);
}

static inline mat3 *mat3_mul_VERSION_A(mat3 *result, const mat3 *const mat_A, const mat3 *const mat_B)
{
...
static inline mat4 *mat4_mul_VERSION_A(mat4 *result, const mat4 *const mat_A, const mat4 *const mat_B)
{
...
static inline mat4 mat4_mul_VERSION_B(const mat4 *const mat_A, const mat4 *const mat_B)
{
    mat4 result;
...
static inline vec3 *mat3_vec3_mul_VERSION_A(vec3 *result, const mat3 *const mat, const vec3 *const v)
{
...
static inline vec4 *mat4_vec4_mul_VERSION_A(vec4 *result, const mat4 *const mat, const vec4 *const v)
{
